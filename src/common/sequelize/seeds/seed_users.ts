...
export async function runSeedUsers() {
    ...
    const users = [
        { name: 'Admin', email: 'admin@example.com', password: 'adminpass', role: 'admin', balance: '10000.00' },
    ...
    for (const u of users) {
        ...
        if (!exists) {
            ...
            await User.create({ name: u.name, email: u.email, password: hashed, balance: u.balance ?? '3000.00', role: u.role ?? 'user' } as User);
        }
    }

...
describe('TransactionRepository', () => {
    ...
    it('finds by id', async () => {
        const tx = { id: 't1' };
        (TxModel as any).TransactionModel.findByPk = jest.fn().mockResolvedValue(tx);
    ...
    it('creates with options', async () => {
        ...
        (TxModel as any).TransactionModel.create = jest.fn().mockResolvedValue(payload);
    ...
    it('findByUser returns paginated shape', async () => {
        const rows = [{ id: 'a' }];
    (TxModel as any).TransactionModel.findAndCountAll = jest.fn().mockResolvedValue({ rows, count: 1 });

...
export class AdminOrMeGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext) {
        ...
        const resourceId = params['id'] || params['userId'] || params['accountId'];
        if (!resourceId) {
            throw new ForbiddenException('Missing resource identifier for ownership check');
        }

        if (user.sub !== resourceId) {
            throw new ForbiddenException('You can only operate your own resource');
        }
